...
Para criar qualquer cobrança Pix você vai precisar:

<Steps>
  <Step title="Autenticação">
    Ter uma API Key válida (sandbox ou produção) e saber como autenticar suas requisições. Veja o guia de [Autenticação](/start-here/authentication).
  </Step>
  <Step title="Webhook configurado (recomendado)">
    Configurar uma URL de webhook para receber a notificação `PIX_RECEIVED` em tempo real. Veja [Configurando Webhooks](/guias/webhooks/visao-geral).
  </Step>
  <Step title="Ambiente de testes">
    Usar o ambiente sandbox para testar a criação e a simulação de pagamento antes de ir para produção. Veja [Testando no Sandbox](/guides/pix/testando-no-sandbox).
  </Step>
</Steps>
